...
func TestTaskContextCompactionReplacesOldPromptObservationsOnly(t *testing.T) {
	...
	if !taskEventsContain(services.taskEventService.ListTaskEvent("task-1"), agentcontract.TaskEventAgentContextSummary, "rolled summary") {
...
func numberedContextSummaryObservations(count int, contentSize int, markerPrefix string) []turnObservation {
	...
	for index := 1; index <= count; index++ {
		...
		observation := newContentObservation(nextObservationID(index), "continue", "bash", marker+" "+strings.Repeat("x", contentSize))
