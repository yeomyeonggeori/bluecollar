...
func (agentTurnRunner *AgentTurnRunner) recordToolObservation(taskRunID string, state *agentTaskState, actionDocument turnActionDocument, successfulToolCalls map[string]turnObservation, observation turnObservation, recoveryStep string) {
	...
	if observation.RepeatsObservationID != "" {
		agentTurnRunner.appendEvent(taskRunID, agentcontract.TaskEventAgentIdenticalOutput, marshalEventBody(map[string]any{
	...
	if reminder, hasReminder := unchangedResultReminderObservation(state.Request.ToolSet, state.Observations, observation); hasReminder {
		...
		agentTurnRunner.appendEvent(taskRunID, agentcontract.TaskEventAgentUnchangedResult, marshalEventBody(reminderEventBody{
	...
	if reminder, consecutiveCount, hasReminder := toolRepeatReminderObservation(state.Observations, observation); hasReminder {
		state.Observations = append(state.Observations, reminder)
		agentTurnRunner.appendEvent(taskRunID, agentcontract.TaskEventAgentRepeatedToolCall, marshalEventBody(reminderEventBody{
			Observation:         reminder,
	...
	if observation.Failed() {
		agentTurnRunner.appendEvent(taskRunID, agentcontract.TaskEventAgentFailureDebtCreated, marshalEventBody(activeFailureDebtEventBody(state.Observations, agentTurnRunner.options.RecoveryBudget)))
		if recoveryAttemptCount(state.Observations) < agentTurnRunner.options.RecoveryAttemptLimit {
			...
			agentTurnRunner.appendEvent(taskRunID, agentcontract.TaskEventAgentRecoveryGuidance, marshalEventBody(recoveryObservation))
		}
		return
	}
...
func (agentTurnRunner *AgentTurnRunner) saveToolObservation(ctx context.Context, taskRunID string, observationID string, assistantText string, modelReasoning string, modelReasoningField string, toolName string, toolID string, toolInput json.RawMessage, observationToolName string, toolInputKey string, toolResult toolcontract.ToolResult, toolIsReadOnly bool, workspaceRootPath string, minimumModifiedAt time.Time, durationMS int64) turnObservation {
	...
	if !isError && toolcontract.IsArtifactDeliveryTool(toolName) && len(attachments) > 0 {
		...
		if !validityState.Passed {
			...
			attachments = nil
			agentTurnRunner.appendEvent(taskRunID, agentcontract.TaskEventAgentArtifactAttachRejected, marshalEventBody(validityState))
		}
	}
	...
	agentTurnRunner.appendEvent(taskRunID, agentcontract.ToolTaskEventName(toolName, agentcontract.ToolTaskEventResultSuffix), marshalEventBody(observation))
	return observation
}
...
func (agentTurnRunner *AgentTurnRunner) recordToolCrash(taskRunID string, observationID string, toolName string, toolResult toolcontract.ToolResult) {
	...
	agentTurnRunner.appendEvent(taskRunID, agentcontract.TaskEventToolCrashed, marshalEventBody(map[string]any{
		"observationID": observationID,
