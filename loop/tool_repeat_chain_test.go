...
func TestBookkeepingBetweenTwoIdenticalCallsDoesNotLaunderTheLoop(t *testing.T) {
	searchKey := "bash\x00{\"command\":\"grep -r needle .\"}"
	observations := []turnObservation{
		callObservation("obs-1", toolcontract.BashToolName, searchKey),
		callObservation("obs-2", toolcontract.PlanToolName, "plan\x00{\"steps\":[]}"),
		callObservation("obs-3", toolcontract.BashToolName, searchKey),
	}
...
func TestADifferentCallBreaksTheChain(t *testing.T) {
	...
	observations := []turnObservation{
		callObservation("obs-1", toolcontract.BashToolName, searchKey),
		callObservation("obs-2", toolcontract.BashToolName, "bash\x00{\"command\":\"ls\"}"),
		callObservation("obs-3", toolcontract.BashToolName, searchKey),
	}
...
func TestACallThatKeptFailingStillCounts(t *testing.T) {
	deniedKey := "write\x00{\"path\":\"/etc/passwd\"}"
	denied := callObservation("obs-1", toolcontract.WriteToolName, deniedKey)
...
func TestTheThirdIdenticalCallEarnsANudge(t *testing.T) {
	searchKey := "bash\x00{\"command\":\"grep -r needle .\"}"
	call := callObservation("obs-3", toolcontract.BashToolName, searchKey)
	observations := []turnObservation{
		callObservation("obs-1", toolcontract.BashToolName, searchKey),
		callObservation("obs-2", toolcontract.BashToolName, searchKey),
		call,
	}
...
func TestTheSecondIdenticalCallIsLeftAlone(t *testing.T) {
	searchKey := "bash\x00{\"command\":\"grep -r needle .\"}"
	call := callObservation("obs-2", toolcontract.BashToolName, searchKey)
	observations := []turnObservation{callObservation("obs-1", toolcontract.BashToolName, searchKey), call}
...
func TestALongInputIsQuotedShortButComparedWhole(t *testing.T) {
	longInput := "write\x00{\"text\":\"" + strings.Repeat("x", 4000) + "\"}"
	call := callObservation("obs-5", toolcontract.WriteToolName, longInput)
...
func toolSetDeclaringPlanUpdateWithoutSideEffect(t *testing.T) *toolcontract.ToolSet {
	...
	return newTestToolSetWithDefinitions([]toolcontract.ToolDefinition{
		...
		{Name: toolcontract.BashToolName, SideEffectClass: toolcontract.ToolSideEffectStateChange, Visibility: toolcontract.ToolVisibilityModel},
	})
}
...
func TestARepeatedResultFromAToolThatDoesSomethingIsNotANoOp(t *testing.T) {
	toolSet := toolSetDeclaringPlanUpdateWithoutSideEffect(t)
	repeated := callObservation("obs-3", toolcontract.BashToolName, "bash\x00{\"command\":\"ls\"}")
	repeated.RepeatsObservationID = "obs-1"
...
func TestTheThirdIdenticalResultFromASideEffectingToolIsALoop(t *testing.T) {
	toolSet := toolSetDeclaringPlanUpdateWithoutSideEffect(t)
	observations := []turnObservation{
		callObservation("obs-1", toolcontract.BashToolName, "bash\x00{}"),
		resultRepeating("obs-2", toolcontract.BashToolName, "obs-1"),
	}
	third := resultRepeating("obs-3", toolcontract.BashToolName, "obs-1")

	if _, hasReminder := unchangedResultReminderObservation(toolSet, observations, resultRepeating("obs-2", toolcontract.BashToolName, "obs-1")); hasReminder {
