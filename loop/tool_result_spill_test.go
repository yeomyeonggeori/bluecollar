...
func TestTheWholeOutputIsSavedWhereTheAgentCanReadItBack(t *testing.T) {
	...
	observation := services.runner.saveToolObservation(context.Background(), "run-1", "obs-1", "", "", "", "bash", "tool-1",
		nil, "bash", "bash\x00{}", toolcontract.ToolResult{Output: toolcontract.ToolOutput{Content: content}},
...
func TestAFailedSaveIsRecordedSoTheGapIsExplainable(t *testing.T) {
	...
	services.runner.spillToolResult(context.Background(), "run-1", "obs-1", "bash", "/workspace", "output")
...
func TestTheSavedObservationKeepsTheReasoningThatChoseIt(t *testing.T) {
	...
	observation := services.runner.saveToolObservation(context.Background(), "run-1", "obs-1", reasoning, "", "", "bash", "tool-1",
		nil, "bash", "bash\x00{}", toolcontract.ToolResult{Output: toolcontract.ToolOutput{Content: "ok"}},
		true, "/workspace", time.Time{}, 12)
