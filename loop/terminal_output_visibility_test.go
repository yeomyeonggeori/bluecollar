...
func TestATerminalSummaryNeverReplacesOutputWithAnExitCode(t *testing.T) {
	observation := shellObservation(`{"exitCode":0,"output":"Commands: login search_contacts send_text_message"}`)
...
func TestATerminalSummaryKeepsTheOutputItCanRead(t *testing.T) {
	observation := shellObservation(`{"exitCode":0,"stdout":"Commands: login search_contacts send_text_message"}`)
