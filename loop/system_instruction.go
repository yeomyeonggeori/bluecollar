...
func buildAgentSystemInstruction(request AgentTurnRequest, options TurnOptions) SystemInstruction {
	...
	systemInstruction = systemInstruction.Append("tool_calling",
		"Tool calling: The action schema contains the exact tools callable in this step. Call domain operations directly by name with their typed parameters. When the tool you need is not in hand, tell equip what you need and call what it returns on your next step; never guess a tool name or run an agent tool name as a shell command. The runtime injects requester identity, approval, and delivery — never pass requester identity in input."+
...
func missingCapabilityClaimInstruction() string {
	return " Before you tell the user you lack the capability, permission, access, or data, tell equip what you need and actually try what it returns. Only claim you cannot do something after a real attempt comes up empty, and say what you tried."
}
