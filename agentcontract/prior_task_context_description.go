...
func PriorTaskContextDescription(context PriorTaskContext) string {
	...
	return strings.Join([]string{
		...
		"This is context for interpreting the latest user message, not permission to close from old text. If the latest user message asks to deliver, retry, continue, or revise this prior task's outcome, set priorTaskReference=outcome_recovery. If it is unrelated or self-contained, set priorTaskReference=none. When recovering an outcome, infer the needed structured output formats from the prior task prompt, prior result, known contract, and latest user message. A file deliverable reaches the user only when the final reply of the current task attaches it; a prepared file, generated path, task link, or prior result text is not delivery.",
	}, "\n")
}
